...
// EShare reserve contract1
contract EShareReserve {
    ...
    function EShareReserve( address _eshareNetwork, address _reserveOwner ) {
    ...
    bool validate ) internal returns(bool) {
        ...
        // can skip validation if done at eshare network level
        if( validate ) {
    ...
    bool validate ) payable returns(bool) {
        ...
        if( msg.sender != eshareNetwork ) {
            // sender must be eshare network
    ...
    /// @param validate If true, additional validations are applicable
    /// @return true iff trade is succesful destToken = conversionRatesx source token
    ...
    /// @return true iff deposit is succesful
    function depositDGXToken( ERC20 token, uint amount ) returns(bool) {
